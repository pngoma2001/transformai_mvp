...
st.sidebar.caption("PE portfolio transformation demo")
...
st.title("TransformAI — Portfolio Company Analyzer")
...
if st.button("Analyze Company", type="primary"):
    ...
    with st.spinner("Analyzing…"):
        time.sleep(0.6)
        result: AnalysisResult = analyze(df)
...
if "last_result" in st.session_state:
    ...
    if st.button("Export 1-page PDF (mock HTML)"):
        # For simplicity, export HTML that can be printed to PDF
        html = f"""
