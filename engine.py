...
from models import AnalysisResult, Play, PlanStep
import numpy as np
...
def pricing_play(kpis: Dict[str, Any]) -> Play:
    rev = kpis["revenue"]
    gm = kpis.get("gross_margin", 0.35) or 0.35
    adoption = 0.6  # assumed adoption of new prices on targeted SKUs
    price_lift = 0.06  # mid-point 6%
    ...
    return Play(
        ...
        plan=[
            PlanStep(week=1, step="Price test cohorts by SKU decile", owner_role="RevOps"),
            PlanStep(week=2, step="Roll to 30% of traffic; monitor conversion", owner_role="Ecomm"),
            PlanStep(week=3, step="Expand to 70%; adjust per elasticity", owner_role="GM"),
            PlanStep(week=4, step="Full rollout; set guardrails", owner_role="Finance"),
        ]
    )

def retention_play(kpis: Dict[str, Any]) -> Play:
    rev = kpis["revenue"]
    gm = kpis.get("gross_margin", 0.35) or 0.35
    churn = kpis.get("churn_rate", 0.2) or 0.2
    reduce_pp = 0.03  # 3 percentage points reduction
    ...
    return Play(
        ...
        plan=[
            PlanStep(week=1, step="Segment cohorts by LTV/RFM", owner_role="CRM"),
            PlanStep(week=2, step="Launch win-back & VIP offers", owner_role="Marketing"),
            PlanStep(week=3, step="Automate journeys in Salesforce", owner_role="CRM"),
            PlanStep(week=4, step="Readout & scale", owner_role="Ops"),
        ]
    )

def inventory_play(kpis: Dict[str, Any]) -> Play:
    rev = kpis["revenue"]
    gm = kpis.get("gross_margin", 0.35) or 0.35
    uplift = rev * 0.01 * gm  # recover 1% lost sales
    return Play(
        id="supply1",
        type="supply",
        ...
        plan=[
            PlanStep(week=1, step="Flag SKUs with frequent stockouts", owner_role="Supply Chain"),
            PlanStep(week=2, step="Set reorder points & buffers", owner_role="Ops"),
            PlanStep(week=3, step="Pilot auto-PO flow", owner_role="Procurement"),
            PlanStep(week=4, step="Scale across categories", owner_role="COO"),
        ]
    )

def utilization_play(kpis: Dict[str, Any]) -> Play:
    rev = kpis["revenue"]
    gm = kpis.get("gross_margin", 0.33) or 0.33
    uplift = rev * 0.02 * gm # +2% utilization
    return Play(
        id="util1",
        type="utilization",
        ...
        plan=[
            PlanStep(week=1, step="Gap analysis of schedule grid", owner_role="Ops"),
            PlanStep(week=2, step="Enable overbooking rules + SMS", owner_role="IT"),
            PlanStep(week=3, step="Optimize templates per location", owner_role="Ops"),
            PlanStep(week=4, step="Measure & lock SOP", owner_role="COO"),
        ]
    )
...
def analyze(df: pd.DataFrame) -> AnalysisResult:
    ...
    if "retail" in company:
        ...
        # generic
        plays = [pricing_play(kpis), retention_play(kpis), inventory_play(kpis)]
    return AnalysisResult(kpis=kpis, plays=plays)
